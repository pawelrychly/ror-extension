##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < postFactumPreferenceRelatedEvalModyfying.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < postFactumPreferenceRelatedEvalModyfying.R
##########################################

inDirectory <- commandArgs()[5] # "D:/projects/praca-magisterska/ROR-ranking/services/postFactumPreferenceRelatedEvalModifying-PutPoznan/tests/in3" # commandArgs()[5] #
outDirectory <- commandArgs()[6] #"D:/projects/praca-magisterska/ROR-ranking/services/postFactumPreferenceRelatedEvalModifying-PutPoznan/tests/out3" # commandArgs()[6] # 
...
PfaBuildConstraints <- function(perf, a, b, is.possible=TRUE, strict.vf = FALSE,
  ...
  all.constraints <-ror:::combineConstraints(base.model, a.pref.b)
  return(all.constraints)
}
...
BinarySearchForPossible <- function(perf, a, b, is.possible=TRUE, strict.vf, strong.prefs=NULL,weak.prefs=NULL, indif.prefs = NULL,
  ...
  repeat {
    ...
    constraints <- PfaBuildConstraints(perf = new.perf, a = a, b = b, is.possible = is.possible, strict.vf = strict.vf,
    ...
    if (CheckConstraintsConsistency(new.perf, constraints)){       
      new.q <- q - (diff / 2)
      diff <- q - new.q
...
BinarySearchForNecessary <- function(perf, a, b, is.possible=FALSE, strict.vf, strong.prefs=NULL,weak.prefs=NULL, indif.prefs = NULL,
                                     nums.of.characteristic.points=NULL, which.attributes = NULL, precision=0.00005, start.q) {
...
FindQLimes <- function(perf, a, b=NULL,  start.q, is.possible=FALSE, strict.vf,
                       strong.prefs=NULL,weak.prefs=NULL, indif.prefs = NULL,
                       nums.of.characteristic.points=NULL, which.attributes = NULL, direction.up = 1, precision=0.00005){
...
##------------------
#Funkcja inicjuje wartość q, przed rozpoczęciem minimalizacji / maxymalizacji
PfaFindQ <- function(perf, a, b, is.possible=FALSE, strict.vf,
                     strong.prefs=NULL,weak.prefs=NULL, indif.prefs = NULL,
                     nums.of.characteristic.points=NULL, which.attributes = NULL) {
  q <- 0.5 # q jest na początku mnożone, więc zaczynamy od q == 1
  stop.searching = FALSE
  repeat {
    q <- 2*q 
    
    new.perf <- PfaImproveVariant(perf, a, q, which.attributes = which.attributes)
    if (EqualMatrix(new.perf, perf) && q > 1) {
      stop.searching = TRUE  
    }
    ...
    constraints <- PfaBuildConstraints(perf = new.perf, a = a, b = b, is.possible = is.possible, strict.vf = strict.vf,
                                       strong.prefs = strong.prefs, weak.prefs = weak.prefs, indif.prefs = indif.prefs,
                                       nums.of.characteristic.points = nums.of.characteristic.points)
    result <- CheckConstraintsConsistency(new.perf, constraints)
    if ((is.possible) && (result)){  # Dla "możliwej" jest minimalizacja. Więc w tym wypadku zwiększamy najpierw q, dopóki
      # ograniczenia nie będą spójne. ( szukamy dowolnej liczby większej, którą następnie
      # będziemy minimalizować.)
      break
    } 
    if ((is.possible) && (!result) && (stop.searching)) {
...
PfaFindQPossibleImprovement <- function(perf, a, b, strict.vf,
  ...
  repeat {    
    ...
    constraints <- PfaBuildConstraints(perf = new.perf, a = a, b = b, is.possible = TRUE, strict.vf = strict.vf,
                                       strong.prefs = strong.prefs, weak.prefs = weak.prefs, indif.prefs = indif.prefs,
                                       nums.of.characteristic.points = nums.of.characteristic.points)
    ...
    if (result) {
      ...
      q <- BinarySearchForPossible(perf = perf, a = a, b = b, is.possible = TRUE, strict.vf = strict.vf,
                                   strong.prefs = strong.prefs, weak.prefs = weak.prefs, indif.prefs = indif.prefs,
                                   nums.of.characteristic.points = nums.of.characteristic.points,
  ...
  repeat {  
    ...
    if (!result) {
      ...
      if (stop.searching) {
        ...
        return(
          FindQLimes( perf, a, b, start.q = q, is.possible=TRUE, strict.vf=strict.vf, which.attributes = which.attributes,
                      direction.up = 1, precision=precision, nums.of.characteristic.points = nums.of.characteristic.points)) ##search limes
...
PfaFindQNecessaryImprovement <- function(perf, a, b, strict.vf,
  ...
  repeat {    
    ...
    constraints <- PfaBuildConstraints(perf = new.perf, a = a, b = b, is.possible = FALSE, strict.vf = strict.vf,
                                       strong.prefs = strong.prefs, weak.prefs = weak.prefs, indif.prefs = indif.prefs,
                                       nums.of.characteristic.points = nums.of.characteristic.points)
    ...
    if (!result) {
      ...
      q <- BinarySearchForNecessary(perf = perf, a = a, b = b, is.possible = FALSE, strict.vf = strict.vf,
                                    strong.prefs = strong.prefs, weak.prefs = weak.prefs, indif.prefs = indif.prefs,
                                    nums.of.characteristic.points = nums.of.characteristic.points,
                                    which.attributes = which.attributes, precision=precision, start.q = q)
      return(q);#binary search
      ...
      if (stop.searching) {
        #wydaje mi się, że tutaj powinno być infeasible, gdyż a nigdy nie będzie koniecznie preferowany nad b
        stop("infeasible")
  ...
  repeat {    
    ...
    if (result) {
      ...
      if (stop.searching) {
        ...
        return(
          FindQLimes( perf, a=a, b=b,  start.q = q, is.possible=FALSE, strict.vf=strict.vf, which.attributes = which.attributes,
                      direction.up = 1, precision=precision, , nums.of.characteristic.points = nums.of.characteristic.points)) ##search limes
      }
    }
...
#############################################################################################
########################### Preference Related Eval Modifying ###############################
#############################################################################################


#POST FACTUM ANALYSIS - Powiązane funkcje, znajdują się w pliku postFactumAnalysus.R. 
#         min q >= 1 lub min q < 1
# Funkcja znajduje odpowiedzi na pytania:
#   1.) Jak mało muszę się polepszyć by coś możliwie zachodziło (greater.than.one = TRUE - q >= 1 )
#   2.) Jak dużo mogę się pogorszyć, by coś wciąż możliwie zachodziło (greather.than.one = FALSE - q <= 1)
# a - indeks (numer wiersza) wariantu a  
# b - indeks (numer wiersza) wariantu b
# which.attributes - lista zer i jedynek określająca, które atrybuty mają być brane pod uwagę, podczas mnożenia przez q
#                    1 oznacza wybór atrybutu ( przykład: c(1,0,1,0) oznacza, że 1 i 3 atrybut będą modyfikowane)
# Przykład wywołania: 
#   q.p.greater <- PfaPossibleComprehensiveImprovement(perf = preferences, 3, 4, strict.vf = TRUE, precision=0.00005, which.attributes = NULL, greater.than.one = TRUE)
#
PfaPossibleComprehensiveImprovement <- function(perf, a, b, strict.vf, strong.prefs=NULL,
  ...
  if (greater.than.one) {
    q <- PfaFindQPossibleImprovement(perf = perf, a=a, b=b, strict.vf=strict.vf,
                                     strong.prefs=strong.prefs,weak.prefs=weak.prefs, indif.prefs = indif.prefs,
                                     nums.of.characteristic.points=nums.of.characteristic.points, which.attributes = which.attributes, precision=0.00005) 
  } else {
...
# Funkcja znajduje odpowiedzi na pytania:
#   1.) Jak dużo muszę się poprawiać by możliwie zachodziło przeciwne do tego co sprawdzamy (greater.than.one = TRUE - q >= 1 )
#   2.) Jak mało muszę się pogorszyć by możliwie zachodziło przeciwne do tego co sprawdzamy (greather.than.one = FALSE - q <= 1)
# a - indeks (numer wiersza) wariantu a  
# b - indeks (numer wiersza) wariantu b
# which.attributes - lista zer i jedynek określająca, które atrybuty mają być brane pod uwagę, podczas mnożenia przez q
#                    1 oznacza wybór atrybutu ( przykład: c(1,0,1,0) oznacza, że 1 i 3 atrybut będą modyfikowane)
...
#   q.p.greater <- PfaPossibleComprehensiveImprovement(perf = preferences, 3, 4, strict.vf = TRUE, precision=0.00005, which.attributes = NULL, greater.than.one = TRUE)
#
PfaNecessaryComprehensiveImprovement <- function(perf, a, b, strict.vf, strong.prefs=NULL,
                                                 weak.prefs=NULL, indif.prefs = NULL, nums.of.characteristic.points=NULL, precision=0.00005, 
                                                 which.attributes = NULL, greater.than.one = TRUE) {
  # greater than one  = TRUE q >= 1 else q < 1  
  ...
  tmpErr<- try (
    {
      ...
      if (parameters[["is-possible-comprehensive-modifying"]]) {
        result <- PfaPossibleComprehensiveImprovement(perf=performances, a = targetRelation[1], b = targetRelation[2], strict.vf = parameters[["strict"]], strong.prefs = pref$strong, weak.prefs=pref$weak, indif.prefs=pref$indif, nums.of.characteristic.points=cp, precision=parameters[["precision"]], 
                                                                    which.attributes = which.attributes, greater.than.one = parameters[["is-improvement"]])
        
      } else {
        result <- PfaNecessaryComprehensiveImprovement(perf=performances, a = targetRelation[1], b = targetRelation[2], strict.vf = parameters[["strict"]], strong.prefs = pref$strong, weak.prefs=pref$weak, indif.prefs=pref$indif, nums.of.characteristic.points=cp, precision=parameters[["precision"]], 
                                                                    which.attributes = which.attributes, greater.than.one = parameters[["is-improvement"]])  
