##########################################
# Usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < postFactumRankRelatedUtilityModyfying.R
# Example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < postFactumRankRelatedUtilityModyfying.R
##########################################

inDirectory <- commandArgs()[5] # "D:/projects/praca-magisterska/ROR-ranking/services/postFactumRankRelatedUtilityModifying-PutPoznan/tests/in1" # 
outDirectory <- commandArgs()[6] #  "D:/projects/praca-magisterska/ROR-ranking/services/postFactumRankRelatedUtilityModifying-PutPoznan/tests/out1" # 
...
#############################################################################################
######################## Rank Related Utility Modifying #####################################
#############################################################################################


# Rank Related Analysis - Funkcja realizuje podejście z brakującą użytecznością U_x:
#   1.) Jak wiele użyteczności trzeba dodać do wariantu a by osiągnało ono MOŻLIWIE co najmniej k tą pozycję w rankingu  ( improvement == TRUE and is.possibly.preffered == TRUE ) 
#   2.) Jak dużo można odebrać użyteczności wariantowi a, by mimo to nadal był on MOŻLIWIE co najmniej na k tej pozycji w rankingu ( improvement == FALSE and is.possibly.preffered == TRUE )
#   1.) Jak wiele użyteczności trzeba dodać do wariantu a by osiągnało ono KONIECZNIE co najmniej k tą pozycję w rankingu  ( improvement == TRUE and is.possibly.preffered == FALSE )
#   2.) Jak wiele użyteczności można odjąć od a by nadal był on KONIECZNIE na co najmniej k tej pozycji w rankingu ( improvement == FALSE and is.possibly.preffered == FALSE )
#   
#   Przykład wywołania: Umiss <-PfaUmissingNecessaryOrPossibleComprehensiveRankImprovement(perf=performances, strong.prefs=str, a = 4, b = 3, strict.vf = TRUE, is.possibly.preffered = TRUE, nums.of.characteristic.points=c(4,4), improvement=FALSE)
#
#   Przy tej metodzie aktualnie dałem roboczo wyświetlanie rozwiązania równania (ostatnia wypisana linijka), tj między innymi wartości funkcji użyteczności w poszczególnych
#   punktach. Może to być pomocne przy analizie rozwiązania. Postaram się później zrobić to ładniej. Narazie to zostawiam, bo boję się, że nie 
#   zdąże do czwartku wszystkiego napisać. 
#
PfaUmissingNecessaryOrPossibleComprehensiveRankImprovement <- function(perf, a, k, strict.vf, is.possibly.preffered,
  ...
  ret <- PfaRanksSolveModelForU(performances =perf, model=constraints, maximum=  maximum)
  #Show solution
  number.of.binary.variables <- nrow(perf)
  ShowSolutionForRanks(perf=perf, number.of.binary.variables = number.of.binary.variables,  nums.of.characteristic.points = nums.of.characteristic.points, ret=ret)
  #print(constraints)
  return(ret$objval)
}
...
if (is.null(performances)) {
  ...
  targetRanking <- getTarget(args, performances)
  if (is.null(targetRanking)) {
    is_proper_data = FALSE
  }
}
  ...
  tmpErr<- try (
{
  ...
  print(targetRanking)
  print(performances)
  result <- PfaUmissingNecessaryOrPossibleComprehensiveRankImprovement(perf=performances, a = targetRanking[1], k = targetRanking[2], strict.vf = parameters[["strict"]], is.possibly.preffered = parameters[["is-possible-comprehensive-modifying"]],
...
if (execFlag) {
  ...
  putAlternativeValue(outTree, result, alternativesIDs = ids[targetRanking[1]], mcdaConcept="utility-modifying")
